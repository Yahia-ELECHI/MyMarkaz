...
class CloudflareD1Client:
    ...
    def execute_query(self, sql, params=None):
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"Erreur D1 (HTTP {response.status_code}): {response.text}")
            ...
            if not result.get('success', True):
                ...
                raise Exception(f"Erreur D1: {errors}")
                
            return result.get('result', [])
            
        except requests.exceptions.RequestException as e:
            raise Exception(f"Erreur de connexion D1: {str(e)}")
    ...
    def execute_batch(self, queries):
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"Erreur D1 batch (HTTP {response.status_code}): {response.text}")
                
            return response.json()
            
        except requests.exceptions.RequestException as e:
            raise Exception(f"Erreur de connexion D1 batch: {str(e)}")
