...
def appel_rapide_professeur(request):
    ...
    if not classe_id and classes.exists():
        ...
        base_url = reverse('appel_rapide_professeur')
        return redirect(f"{base_url}?classe={selected_classe.id}&date={selected_date.strftime('%Y-%m-%d')}")
    ...
    if request.method == 'POST':
        ...
        if action == 'save_all_presences':
            ...
            try:
                # Appeler la fonction de sauvegarde des présences
                result = handle_save_all_presences(request, selected_classe, selected_date)
                # Si c'est une requête AJAX, retourner le résultat JSON
                if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                    return result
                # Sinon, rediriger avec un message de succès
                messages.success(request, f"Les présences ont été enregistrées avec succès.")
                return redirect('appel_rapide_professeur')
            except Exception as e:
                messages.error(request, f"Erreur lors de l'enregistrement des présences: {str(e)}")
                return redirect('appel_rapide_professeur')
...
def handle_save_all_presences(request, selected_classe, selected_date):
    """Gère la sauvegarde de toutes les présences pour une classe et une date - Version simplifiée"""
    ...
    try:
        ...
        eleves = selected_classe.eleves.all()
        saved_count = 0
        ...
        for key, value in request.POST.items():
            if key.startswith('presence_'):
                ...
                presence_data[eleve_id] = {
                    'status': value,
                    'comment': request.POST.get(f'comment_{eleve_id}', '')
                }
        
        # Traiter chaque élève
        for eleve in eleves:
            eleve_id = str(eleve.id)
            if eleve_id in presence_data:
                data = presence_data[eleve_id]
                status = data['status']
                commentaire = data['comment']
                
                # Déterminer le statut de présence
                present = status == 'present'
                justifie = status == 'absent-justified'
                
                # Créer ou mettre à jour l'enregistrement de présence
                PresenceEleve.objects.update_or_create(
                    eleve=eleve,
                    date=selected_date,
                    classe=selected_classe,
                    defaults={
...
def gestion_eleves_professeur(request):
    ...
    context = {
        ...
        'eleves': eleves,
        'all_classes': all_classes,  # Toutes les classes de la composante
        'today': timezone.now().date(),  # Ajouter la date du jour
    }
