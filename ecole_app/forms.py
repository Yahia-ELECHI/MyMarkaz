...
class EcouteAvantMemoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date', 'value': timezone.now().strftime('%Y-%m-%d')}),
            'debut_page': forms.Select(attrs={'class': 'form-control', 'id': 'debut-page-select-ecoute', 'disabled': 'disabled'}),
            'fin_page': forms.Select(attrs={'class': 'form-control', 'id': 'fin-page-select-ecoute', 'disabled': 'disabled'}),
...
class MemorisationForm(forms.ModelForm):
    ...
    sourate_select = forms.ChoiceField(
        choices=get_sourates_choices,
        widget=forms.Select(attrs={'class': 'form-control', 'id': 'id_sourate_select'}),
        label="Sourate"
    )
    sourate = forms.CharField(widget=forms.HiddenInput(), required=False)
    ...
    class Meta:
        ...
        widgets = {
            'date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date', 'value': timezone.now().strftime('%Y-%m-%d')}),
            'debut_page': forms.Select(attrs={'class': 'form-control', 'id': 'id_debut_page'}),
            'fin_page': forms.Select(attrs={'class': 'form-control', 'id': 'id_fin_page'}),
            'enseignant': forms.Select(attrs={'class': 'form-control'}),
            'remarques': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Remarques (facultatif)', 'rows': 2}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Si nous avons une instance existante, essayer de déterminer la sourate à partir des pages
        elif self.instance and self.instance.pk:
            try:
                from .sourate import SOURATES, get_sourate_for_page
                # Déterminer la sourate à partir de la page de début
                if hasattr(self.instance, 'debut_page'):
                    sourate_index = get_sourate_for_page(self.instance.debut_page)
                    if sourate_index is not None:
                        self.initial['sourate_select'] = str(sourate_index)
                        self.initial['sourate'] = SOURATES[sourate_index].nom
...
class RevisionForm(forms.ModelForm):
    """Formulaire pour ajouter/modifier une révision"""
    
    class Meta:
        model = Revision
        fields = ['date', 'nombre_hizb', 'remarques']
        widgets = {
            'date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
            'nombre_hizb': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.25', 'min': '0'}),
            'remarques': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Remarques (facultatif)', 'rows': 2})
        }
        
    def save(self, commit=True):
        instance = super().save(commit=False)
        # Auto-populate jour field from date
        if instance.date:
            # Get the weekday name in French
            weekday = instance.date.weekday()
            days = ['lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi', 'dimanche']
            instance.jour = days[weekday]
            
            # Calculate the week number (ISO week)
            instance.semaine = instance.date.isocalendar()[1]
...
class RepetitionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'page': forms.Select(attrs={'class': 'form-control', 'id': 'id_page'}),
...
class NoteExamenForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.professeur and self.composante_id:
            # Pour l'administrateur (professeur "administration"), afficher toutes les classes de la composante
            if self.professeur.nom == "administration":
                # Afficher toutes les classes de la composante actuelle
                classes = Classe.objects.filter(composante_id=self.composante_id).order_by('nom')
                self.fields['classe'].queryset = classes
                
                # Afficher tous les élèves des classes de la composante actuelle
                eleves = Eleve.objects.filter(
                    classes__composante_id=self.composante_id,
                    archive=False
                ).distinct().order_by('nom', 'prenom')
                self.fields['eleve'].queryset = eleves
            else:
                # Pour les professeurs normaux, garder le comportement existant
