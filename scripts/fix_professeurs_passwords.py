...
def main():
    ...
    count = 0
    for prof in Professeur.objects.filter(mot_de_passe_en_clair__isnull=True):
        if not prof.user:
            continue
        ...
        prof.save()
        print(f"[OK] Mot de passe fixé pour {prof.nom} (id={prof.id}) : {new_password}")
        count += 1
    print(f"Correction terminée. {count} professeurs corrigés.")
